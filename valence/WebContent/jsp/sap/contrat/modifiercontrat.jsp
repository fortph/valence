...
ContratCDI sap=csapdao.findByID(Integer.parseInt(contrats));

IdentiteDAO agdao=new IdentiteDAO();
Identite une=agdao.findByID(sap.getIdentite().getId_IDE());
...
<title>modifier contrat sap</title>
<script type="text/javascript">
...
<body>
	...
	<div id="body">
		...
		<br>
		<div id="creation">MODIFIER CONTRAT SAP C<%=contrats %></div>
		<br>
		<h3>IDENTITE</h3>
		<table class="table1">
		<tr>
		...
		<form name="form1" method="post" action="/valence/controleur" >		
			<input type="hidden" name="action" value="sapmodifiercontrat" />
			<input type="hidden" name="contrat" value="<%=sap.getId_contratcdi() %>" />
		...
		<table class="table1">
		<tr>
		<td class="un">DATE REDACTION CONTRAT :</td><td><input type="text" id="dateredaction" class="de" name="dateredaction" value="<%=sdf.format(sap.getRedaction())%>"/></td>
		</tr>
		<tr>
		<td class="un">DEBUT :</td><td><input type="text" id="datedebut" class="de" name="datedebut" value="<% if(sap.getEmbauche()!=null) out.println(sdf.format(sap.getEmbauche())); %>"></td>
		...
		<td class="quatre"><input type="text" autocomplete="off" name="salaire" id="salaire" value="<%=sap.getSalairehoraire()%>" /></td>
		
		</tr>
		<tr>
		<td>PANIERS :</td>
		<td class="quatre"><input type="text" autocomplete="off" id="panier" name="panier" value="<%=sap.getPanier()%>" ></td>
	
		</tr>
		<tr>
		<td >DEPLACEMENTS :</td>
		<td class="quatre"><input type="text" autocomplete="off" id="dep" name="dep" value="<%=sap.getDeplacement()%>"></td>
		...
		<table class="table1">
		<tr>
		<td class="une">DATE TERME CONTRAT:</td><td><input type="text" name="terme" class="de" id="terme" value="<% if(sap.getTermecontrat()!=null) out.println(sdf.format(sap.getTermecontrat()));%>"/></td>
		</tr>
		</table>
		<br>
		<hr>
