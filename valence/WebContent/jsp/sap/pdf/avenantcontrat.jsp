...
<%
	...
	ContratCDI contratcdi = contratdao.findByID(Integer.parseInt(numerocontrat));

	AvenantCDIDAO contdao = new AvenantCDIDAO();
	AvenantCDI contrat = contdao.findByID(Integer.parseInt(numerocontrat));
	...
	Identite identite = idao.findByID(contrat.getContrat().getIdentite().getId_IDE());
	String civil = "M.";
...
<body>
	...
	<%
		...
		Font rougegras = new Font(Font.FontFamily.TIMES_ROMAN, 18, Font.BOLD);
		rougegras.setColor(BaseColor.RED);
		selector.addFont(rougegras);
		...
		try {
			...
			titre.setLeading(25);
			//titre.add("coucou");
			titre.add(new Phrase("AVENANT AU CONTRAT DE TRAVAIL NUMERO ", titregras));
			titre.add(selector
					.process("A" + contrat.getRangavenent() + "-C" + contrat.getContrat().getId_contratcdi()));
			titre.add(new Phrase("\nà temps partiel modulé, à durée Indéterminée\n\n"
					+ "dans le cadre de l’activité\n'Service à la Personne'\nAgrément simple n° "
					+ contratcdi.getAgrement(), titregras));
			titre.add(new Phrase("\n\nSuite au contrat initial conclu le ", titregras));
			Phrase datecontrat = selector.process(sdf.format(contratcdi.getRedaction()));
			titre.add(datecontrat);
			document.add(titre);
			...
			Phrase ph1 = new Phrase(civil + " " + identite.getPrenom_IDE() + " " + identite.getNom_IDE(),
					policecontrat);
			Phrase ph2 = new Phrase(
					" est engagé(e) dans le cadre d’un Contrat à durée indéterminée à temps partiel", titre1gras);
			...
			Phrase ph3 = new Phrase(civil + " " + identite.getPrenom_IDE() + " " + identite.getNom_IDE()
					+ " s’engage, en outre, à se conformer aux dispositions du règlement intérieur, dont un "
					+ " exemplaire lui a été remis ce jour.", policecontrat);
			conditions1.add(ph1);
			conditions1.add(ph2);
			conditions1.add(ph4);
			conditions1.add(ph3);
			...
			Phrase ph5 = new Phrase(civil + " " + identite.getPrenom_IDE() + " " + identite.getNom_IDE()
					+ " est engagé(e) par l’association " + org.getRs() + " à compter du  "
					+ sdf.format(contrat.getDateeffet()) + " et dans le cadre ", policecontrat);
			Phrase ph6 = new Phrase("d’un contrat de travail à durée indéterminée, à temps partiel modulé.",
					titre1gras);
			objet1.add(ph5);
			objet1.add(ph6);
			...
			Phrase ph7 = new Phrase(civil + " " + identite.getPrenom_IDE() + " " + identite.getNom_IDE()
					+ " occupera un emploi d’ ", policecontrat);
			Phrase ph8 = new Phrase(contrat.getTache(), titre1gras);
			Phrase ph9 = new Phrase(", étant entendu qu’en fonction des nécessités d’organisation du travail,"
			...
			Phrase ph29 = new Phrase(
					"pour s’y rendre dans un périmètre d’intervention des communes appartenant à la Communauté de Communes des Deux Rives.\n\nPar ailleurs, "
							+ civil + " " + identite.getPrenom_IDE() + " " + identite.getNom_IDE()
			...
			Phrase ph30 = new Phrase(civil + " " + identite.getPrenom_IDE() + " " + identite.getNom_IDE(),
					policecontrat);
			Phrase ph31 = new Phrase(" bénéficiera de droits à congés payés conformément aux "
