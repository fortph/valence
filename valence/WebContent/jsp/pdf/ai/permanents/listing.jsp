<%@ page
	import="dao.imp.identite.*,beans.parametres.accueil.*, beans.identite.*,dao.imp.ai.*,
	beans.ai.*,	java.util.*,beans.employeurs.*,dao.imp.employeur.*,divers.*,com.itextpdf.text.*,
...
<%	
	...
	java.util.List<Contrat> listecontrats = new ContratDAO()
			.listingPermanentsContrat(new java.sql.Date(date.getTime()));
	
	java.util.List<Avenant> listeavenants = new AvenantDAO()
	.listingPermanentsAvenant(new java.sql.Date(date.getTime()));
	...
	<%
		...
		if (listecontrats.size() > 0) {
			...
			for (int i = 0; i < listecontrats.size(); i++) {
				...
				int numerocontrat = listecontrats.get(i).getIdaicontrat();
				...
				Contrat derniercontrat= cdao.dernierContratAIEnCours(identite,employeur);
				//on verifie s'il existe des avenants
				Avenant encours=avdao.dernierAvenantContrat(derniercontrat);
				...
				try {
					...
					PdfPCell cel024 = new PdfPCell(new Phrase(listecontrats
							.get(i).getService().getService(), normalgras));
					cel024.setColspan(2);
					cel024.setBorder(12);
					...
					PdfPCell cel0241 = new PdfPCell(new Phrase("",
							normalgras));
					...
					PdfPCell cel044 = new PdfPCell(new Phrase(
							employeur.getCp() + " " + employeur.getVille(),
							normal));
					cel044.setColspan(2);
					cel044.setBorder(12);
					...
					PdfPCell cel054 = new PdfPCell(new Phrase(
							" Téléphone : " + employeur.getTel1(), normal));
					cel054.setColspan(2);
					cel054.setBorder(12);
					...
					PdfPCell qualite4 = new PdfPCell(new Phrase(
							" Poste occupé :   "
									+ listecontrats.get(i).getRome()
											.getIntitule(), normalgras2));
					...
					/******************** deuxieme double tableau *************************/
						...
						if(encours==null){
							sal=derniercontrat.getSalairehoraire();
							pan=derniercontrat.getPanier();
							dep=derniercontrat.getDeplacement();
							fac=derniercontrat.getFacturation();
							div=derniercontrat.getDivers();							
						}
						else{
							sal=encours.getSalairehoraire();
							pan=encours.getPanier();
							dep=encours.getDeplacement();
							fac=encours.getFacturation();
							div=encours.getDivers();
						...
						PdfPCell rem032 = new PdfPCell(new Phrase(pan
								+ " €", normalgras));
						rem032.setBorder(0);
						...
						PdfPCell rem062 = new PdfPCell(new Phrase(div
								+ " €", normalgras));
						rem062.setBorder(0);
						...
						PdfPCell rem072 = new PdfPCell(new Phrase(
								listecontrats.get(i).getCommentaire(), normalgras));
						rem072.setBorder(0);
	...
	document.newPage();
		...
		/*******************edition des avenants sur la periode***********/
			...
			for (int i = 0; i < listeavenants.size(); i++) {
				...
				int numerocontrat = listeavenants.get(i).getIdavenant();
				...
				IdentiteDAO idao = new IdentiteDAO();
				Identite identite = idao.findByID(listeavenants.get(i).getContrat()
						.getIdentite().getId_IDE());
				
				
				
				EmployeurDAO empdao = new EmployeurDAO();
				Employeur employeur = empdao.findByID(listeavenants.get(i).getContrat()
						.getEmployeur().getId_employeur());
				
				
				
				//on récupere le dernier avenant de cette personne
				Avenant dernieravenant=avdao.dernierAvenantPersonne(identite,employeur);
				sal=dernieravenant.getSalairehoraire();
				pan=dernieravenant.getPanier();
				dep=dernieravenant.getDeplacement();
				fac=dernieravenant.getFacturation();
				div=dernieravenant.getDivers();

				//l'employeur peut etre un particulier ou une entreprise 
				String civilite = listeavenants.get(i).getContrat().getEmployeur()
						.getCivemp();
				String affichecivite = "";
				if (civilite != null)
					affichecivite = civilite;
				...
				try {
					...
					PdfPCell cel024 = new PdfPCell(new Phrase(listeavenants.get(i).getContrat().getService().getService(), normalgras));
					cel024.setColspan(2);
					cel024.setBorder(12);
					cel024.setHorizontalAlignment(Element.ALIGN_CENTER);
					table2.addCell(cel024);
					...
					PdfPCell qualite1 = new PdfPCell(new Phrase(
							" Lieu d\'exécution :  "
									+ listeavenants.get(i).getContrat().getLieu(),
							normalgras2));
					qualite1.setColspan(2);
					qualite1.setBorder(Rectangle.NO_BORDER);
					qualite1.setHorizontalAlignment(Element.ALIGN_LEFT);
					...
					PdfPCell qual3 = new PdfPCell(
							new Phrase(" Date de mise à disposition : "
									+ sdf.format(listeavenants.get(i).getContrat()
											.getDebutcontrat()), normalgras2));
					...
					PdfPCell qualite4 = new PdfPCell(new Phrase(
							" Poste occupé :   "
									+ listeavenants.get(i).getContrat().getRome()
											.getIntitule(), normalgras2));
					qualite4.setColspan(2);
					qualite4.setBorder(Rectangle.NO_BORDER);
					qualite4.setHorizontalAlignment(Element.ALIGN_LEFT);
					...
					/******************** deuxieme double tableau *************************/
						...
						rem071.setHorizontalAlignment(Element.ALIGN_LEFT);
						PdfPCell rem072 = new PdfPCell(new Phrase(
								listeavenants.get(i).getContrat().getCommentaire(), normalgras));
						rem072.setBorder(0);
						PdfPCell rem073 = new PdfPCell(new Phrase("Total", normal));
						rem073.setBorder(15);
						PdfPCell rem074 = new PdfPCell(new Phrase("", normal));
