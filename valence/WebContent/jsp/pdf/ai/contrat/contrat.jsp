...
<%
	...
	String periodessai = contdao.calculPeriodeEssai(contrat);
...
<body>
	...
	<%
		...
		try {
			...
			PdfPCell cel4 = new PdfPCell(new Phrase("C"
					+ Integer.valueOf(contrat.getIdaicontrat()).toString(),
					normalgras));
			...
			PdfPCell cel6 = new PdfPCell(new Phrase("", normalgras));
			...
			PdfPCell cel8 = new PdfPCell(new Phrase("", normalgras));
			...
			PdfPCell cel9 = new PdfPCell(new Phrase("CONTRAT DE TRAVAIL\n",
					policecontrat));
			...
			PdfPCell datedebj = new PdfPCell(new Phrase(sdf.format(contrat
					.getDebutcontrat()), normalgras));
			...
			PdfPCell datefinj = new PdfPCell(new Phrase(sdf.format(contrat
					.getFincontrat()), normalgras));
			...
			PdfPCell findureej = new PdfPCell(new Phrase(
					contrat.getHeuresminihebdo() + " heure(s)", normalgras));
			...
			rem01.setColspan(2);
			rem01.setBackgroundColor(new BaseColor(168, 196, 204));
			...
			rem03.setColspan(2);
			rem03.setBackgroundColor(new BaseColor(168, 196, 204));
			...
			PdfPCell rem032 = new PdfPCell(new Phrase(
					decimalformat.format(contrat.getPanier())
					+ " €", normalgras));
			rem031.setBorder(4);
			rem032.setBorder(8);
			...
			PdfPCell rem035 = new PdfPCell(new Phrase(
					decimalformat.format(contrat.getFacturepanier()) + " €", normalgras));
			rem035.setBorder(8);
			...
			PdfPCell rem045 = new PdfPCell(new Phrase(
					decimalformat.format(contrat.getFacturedeplace()) + " €", normalgras));
			rem045.setBorder(8);
			...
			PdfPCell rem055 = new PdfPCell(new Phrase(
					decimalformat.format(contrat.getFacturedivers()) + " €", normalgras));
			rem055.setBorder(10);
			...
			Chunk notes1 = new Chunk(
					" Vous ne pouvez pas démarrer la mission sans que les deux contrats soient signés par l'Association, vous même et l'utilisateur.\n Vous ne pouvez en aucun cas effectuer chez l'utilisateur un autre travail que celui noté sur votre contrat, même si l'utilisateur vous le demande.\n",
					minigras);
			Chunk notes2 = new Chunk(
					" L'Association doit être informée de toute nouvelle tâche demandée, et donner son accord avant l'exécution de celle-ci en effectuant le contrat correspondant.\n",
					minigrasitalic);

			Chunk notes3 = new Chunk(
					" Aucun frais de déplacement ni panier ne sera pris en compte s'il n'est pas prévu sur le contrat.\n",
					minigras);
			Chunk notes4 = new Chunk(
					" Vous vous engagez à ne pas accepter des heures de travail non déclarées chez l'utilisateur.\n",
					minigras);
			Chunk notes5 = new Chunk(
					" Vous devez signaler à l'Association tout problème rencontré en cours de mission.\n",
					minigrasitalic);
			...
			Chunk fait2 = new Chunk(sdf.format(contrat.getRedaction())
					+ ".", normalgras);
			faita.add(fait1);
			faita.add(fait2);
			document.add(faita);

			PdfPTable signatures = new PdfPTable(5);
			signatures.setWidthPercentage(100);//largeur du tableau
			//nombre de colonnes et taille de chacune en proportion
			float[] largeursig = { 3, 6, 1, 3, 6 };
			signatures.setWidths(largeursig);

			PdfPCell sig03 = new PdfPCell(new Phrase(
			...
			PdfPCell sig04 = new PdfPCell(
					new Phrase("Le Salarié :", normal));
			sig04.setPaddingLeft(20);
			sig04.setPaddingTop(7);
			sig04.setBorder(Rectangle.NO_BORDER);
			signatures.addCell(sig04);

			PdfPCell sig05 = new PdfPCell(new Phrase(
					identite.getPrenom_IDE() + " " + identite.getNom_IDE(),
					normalgras));
			sig05.setColspan(2);
			sig05.setPaddingLeft(20);
			sig05.setPaddingTop(7);
			sig05.setBorder(Rectangle.NO_BORDER);
			sig05.setHorizontalAlignment(Element.ALIGN_LEFT);
			signatures.addCell(sig05);

			PdfPCell sig06 = new PdfPCell(new Phrase("L'association : ",
					normal));
			sig06.setPaddingLeft(20);
			sig06.setPaddingTop(7);
			sig06.setBorder(Rectangle.NO_BORDER);
			signatures.addCell(sig06);

			PdfPCell sig07 = new PdfPCell(new Phrase(" CAP EMPLOI\n",
					normalgras));
			sig07.setBorder(Rectangle.NO_BORDER);
			sig07.setPaddingTop(7);
			sig07.setHorizontalAlignment(Element.ALIGN_LEFT);
			signatures.addCell(sig07);

			PdfPCell sig08 = new PdfPCell(
					new Phrase(
							"(Déclare avoir pris connaissance des conditions du contrat): ",
							normal));
			sig08.setColspan(3);
			sig08.setPaddingTop(7);
			sig08.setPaddingLeft(20);
			sig08.setBorder(Rectangle.NO_BORDER);
			signatures.addCell(sig08);

			PdfPCell sig09 = new PdfPCell(new Phrase(
					" (Cachet et signature)", normal));
			sig09.setColspan(2);
			sig09.setPaddingLeft(20);
			sig09.setPaddingTop(7);
			sig09.setBorder(Rectangle.NO_BORDER);
			signatures.addCell(sig09);
