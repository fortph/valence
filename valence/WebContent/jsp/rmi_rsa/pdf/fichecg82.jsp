...
<%
	...
	FicheLiaisonRMI fiche = fldao.findByID(Integer.parseInt(numerofiche));

	ContratRMIDAO contdao = new ContratRMIDAO();
	ContratRMI contrat = contdao.findByID(fiche.getContrat().getId_rmicontrat());
	...
	String presc = fichermi.getPrescripteur();
	//System.out.println("pres ="+presc);
	int numpres = 1;
	if (!presc.equals("Pole Emploi"))
		numpres = 2;
	...
	Organisme cg82 = ordao.findByID(3);
	...
	String intituleAction = "\nAide à la formulation du projet pré-professionnel et évaluation de l'employabilité par la mise en immersion en milieu professionnel.";
%>
...
<body>
	...
	<%
		...
		Font normalrouge = FontFactory.getFont(FontFactory.TIMES_ROMAN, 12f, Font.NORMAL, BaseColor.RED);
		Font normalgras = new Font(Font.FontFamily.TIMES_ROMAN, 12, Font.BOLD);
		...
		Font minigrasitalic = new Font(Font.FontFamily.TIMES_ROMAN, 8, Font.BOLDITALIC);
		...
		Font mapolice = FontFactory.getFont(FontFactory.COURIER, 12f, Font.BOLD, BaseColor.RED);
		//ByteArrayOutputStream buffer = new ByteArrayOutputStream();
		...
		try {
			...
			/*logo de poleemploi
			String urlimagepe = "http://localhost:8080/valence/images/bleu/pe.jpeg";
			...
			//logo du cg82
			String urlimagecg = "http://" + request.getServerName() + ":" + request.getServerPort()
					+ "/valence/images/bleu/tarnetg1.jpg";

			Image cg = new Jpeg(new URL(urlimagecg));
			cg.setAbsolutePosition(20f, 760f); //position de l'image
			cg.scalePercent(33f); //taille de l'image

			//logo de poleemploi
			String urlimagepe = "http://" + request.getServerName() + ":" + request.getServerPort()
					+ "/valence/images/bleu/pe.jpeg";

			Image pe = new Jpeg(new URL(urlimagepe));
			pe.setAbsolutePosition(100f, 770f); //position de l'image
			pe.scalePercent(64f); //taille de l'image

			//logo europe
			String urlimageeur = "http://" + request.getServerName() + ":" + request.getServerPort()
					+ "/valence/images/bleu/logo_Europe1.jpg";

			Image eur = new Jpeg(new URL(urlimageeur));
			eur.setAbsolutePosition(400f, 770f); //position de l'image
			eur.scalePercent(35f); //taille de l'image
			...
			PdfPCell nomtitre = new PdfPCell(new Phrase("FICHE DE LIAISON P.D.I/ RSA\n", normalgras));
			...
			Phrase ph1 = new Phrase("A renvoyer aux prescripteurs et copie :\n", normal);
			...
			Phrase ph6 = new Phrase("Objectifs, Résultats et Perspectives à l'attention du prescripteur\n",
					grosgrasrouge);
			document.add(ph6);
			...
			Phrase ph7 = new Phrase("Orientation pour la période du " + sdf.format(contrat.getDatedeb_rmicontrat())
					+ " au " + sdf.format(contrat.getDatefin_rmicontrat()) + "\n", normalgras);
			Chunk ph8 = new Chunk("( à renseigner par l'opérateur )", normalitalic);
			...
			PdfPCell cel1 = new PdfPCell(new Phrase("Prescripteur : " + fichermi.getPrescripteur(), normalgras));
			cel1.setHorizontalAlignment(Element.ALIGN_CENTER);
			cel1.setBorder(13);

			PdfPCell cel2 = new PdfPCell(new Phrase(
					"Dates du contrat d'Engagement :\nObservations :\n" + fiche.getObsprescripteur(), normal));
			cel2.setRowspan(7);
			PdfPCell cel3 = new PdfPCell(new Phrase("Nom : " + fichermi.getResponsable(), normal));
			cel3.setBorder(12);
			PdfPCell cel4 = new PdfPCell(new Phrase("Fonction : " + fichermi.getFonction(), normal));
			cel4.setBorder(12);
			PdfPCell cel5 = new PdfPCell(new Phrase("Agence ou Pôle : " + fichermi.getAgence(), normal));
			cel5.setBorder(12);
			PdfPCell cel6 = new PdfPCell(
					new Phrase("Nom du référent : " + refepe.getNom() + " " + refepe.getPrenom(), normal));
			cel6.setBorder(12);
			PdfPCell cel7 = new PdfPCell(new Phrase("Mail du référent : " + refepe.getMail(), normal));
			cel7.setBorder(12);
			PdfPCell cel8 = new PdfPCell(new Phrase("* joindre le relevé de conclusion d'entretien", normal));
			cel8.setBorder(14);
			intro.addCell(cel1);
			intro.addCell(cel2);
			intro.addCell(cel3);
			...
			PdfPCell op1 = new PdfPCell(new Phrase("Opérateur : ", normalgras));
			op1.setHorizontalAlignment(Element.ALIGN_CENTER);
			op1.setBorder(13);

			PdfPCell op2 = new PdfPCell(new Phrase("Intitulé action:", normalgras));
			op2.setHorizontalAlignment(Element.ALIGN_CENTER);
			op2.setBorder(13);
			//op2.setRowspan(2);
			PdfPCell op3 = new PdfPCell(new Phrase("Nom : " + cap2000.getRs(), normal));
			op3.setBorder(12);
			PdfPCell op4 = new PdfPCell(new Phrase(intituleAction, normal));
			op4.setBorder(124);
			op4.setRowspan(4);
			PdfPCell op5 = new PdfPCell(new Phrase("Tél : 05.63.29.62.25/26", normal));
			op5.setBorder(12);
			PdfPCell op6 = new PdfPCell(new Phrase("Territoire d'intervention: Valence d'Agen", normal));
			op6.setBorder(12);
			PdfPCell op7 = new PdfPCell(new Phrase("Mail : pdicap2000@mefi-2rives.fr", normal));
			op7.setBorder(124);
			PdfPCell op8 = new PdfPCell(new Phrase("\n", normal));
			op8.setBorder(14);
			PdfPCell op9 = new PdfPCell(new Phrase("\n", normal));
			op9.setBorder(14);
			...
			be2.setRowspan(11);
			PdfPCell be3 = new PdfPCell(new Phrase("Nom : " + identite.getNom_IDE(), normal));
			be3.setBorder(12);
			PdfPCell be4 = new PdfPCell(new Phrase("Prénom : " + identite.getPrenom_IDE(), normal));
			be4.setBorder(12);
			PdfPCell be5 = new PdfPCell(new Phrase("Tél : " + identite.getFixe_IDE(), normal));
			be5.setBorder(12);
			PdfPCell be6 = new PdfPCell(new Phrase("Adresse : " + identite.getAdr1_IDE(), normal));
			be6.setBorder(12);
			PdfPCell be7 = new PdfPCell(
					new Phrase("          " + identite.getCp_IDE() + " " + identite.getVille_IDE(), normal));
			be7.setBorder(12);
			PdfPCell be8 = new PdfPCell(new Phrase("Mail : " + identite.getMail_IDE(), normal));
			be8.setBorder(12);
			PdfPCell be9 = new PdfPCell(new Phrase("Identifiant : " + identite.getPoleEmploi_ID_IDE(), normal));
			be9.setBorder(12);
			PdfPCell be11 = new PdfPCell(new Phrase("Préciser H/F : " + sexe, normal));
			be11.setBorder(12);
			PdfPCell be12 = new PdfPCell(new Phrase("N° CAF ou MSA :", normal));
			be12.setBorder(12);
			...
			document.add(new Paragraph(new Phrase(
					"I - Présence et Objectifs fixés au premier mois de la prise en charge :", normalgras)));
			document.add(
					new Paragraph(new Phrase("A transmettre dans le mois de la prise en charge :", normalrouge)));

			document.add(new Paragraph("\n"));

			PdfPTable pres = new PdfPTable(1);
			pres.setWidthPercentage(100);
			PdfPCell pres1 = new PdfPCell(new Phrase(fiche.getPresence(), normal));
			...
			document.add(new Paragraph(new Phrase("II - Dates des rencontres :", normalgras)));
			...
			document.add(new Paragraph(new Phrase("III - Résultats obtenus : ", normalgras)));
			document.add(new Paragraph(
					new Phrase("A transmettre 15 jours avant la fin de prise en charge :", normalrouge)));
			document.add(new Paragraph("\n"));
			...
			PdfPCell com1 = new PdfPCell(new Phrase("FREINS IDENTIFIES\nen début de parcours", normal));
			com1.setHorizontalAlignment(Element.ALIGN_CENTER);
			com1.setBorder(15);
			com1.setPadding(10);

			PdfPCell com2 = new PdfPCell(new Phrase("FREINS LEVES\nau cours de l'accompagnement", normal));
			com2.setHorizontalAlignment(Element.ALIGN_CENTER);
			com2.setBorder(15);
			com2.setPadding(10);
			PdfPCell com3 = new PdfPCell(new Phrase("FREINS RESTANT A LEVER ", normal));
			com3.setHorizontalAlignment(Element.ALIGN_CENTER);
			com3.setBorder(15);
			com3.setPadding(10);

			PdfPCell com4 = new PdfPCell(new Phrase(fiche.getAcquis(), normal));
			com4.setBorder(12);
			PdfPCell com5 = new PdfPCell(new Phrase(fiche.getEncours(), normal));
			com5.setBorder(12);
			PdfPCell com6 = new PdfPCell(new Phrase(fiche.getNomacquis(), normal));
			com6.setBorder(12);
			...
			document.add(
					new Paragraph(new Phrase("IV - Conclusions de l'opérateur et perspectives :  ", normalgras)));
			document.add(new Paragraph("\n"));
			PdfPTable conclure = new PdfPTable(1);
			conclure.setWidthPercentage(100);
			PdfPCell conc1 = new PdfPCell(new Phrase(fiche.getConclusion(), normal));
