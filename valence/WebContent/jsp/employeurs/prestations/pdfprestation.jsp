...
<%
	...
	PrestationCDIDAO contratdao = new PrestationCDIDAO();
	PrestationCDI prestation = contratdao.findByID(Integer.parseInt(numerocontrat));
	if (!prestation.getCommentaire_pr().equals(""))
		rajoutpaniers = "DIVERS : " + prestation.getCommentaire_pr();

	EmployeurDAO empdao = new EmployeurDAO();
	Employeur employeur = empdao.findByID(prestation.getEmployeur().getId_employeur());

	SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy");

	IdentiteDAO idao = new IdentiteDAO();
	Identite identite = idao.findByID(prestation.getIdentite().getId_IDE());
	...
	<%
		...
		try {
			...
			PdfPCell cel1 = new PdfPCell(
					new Phrase("CONTRAT DE PRESTATIONS DE SERVICE N° P" + numerocontrat, titre));
			...
			Phrase cond12 = new Phrase(
					"une durée hebdomdaire moyenne de : " + prestation.getHeuresminimois_pr() + " h", petitgras);
			cond1.add(cond11);
			cond1.add(cond12);
			document.add(cond1);
			...
			Paragraph arret = new Paragraph(new Phrase(
					" \nEn cas d’arrêt de travail définitif de la part de la salariée mise à disposition, l’association "
							+ org.getRs()
							+ " s’engage à faire intervenir une nouveau salarié à votre domicile dans les mêmes conditions que stipule le contrat.",
					petit));
			...
			Phrase conges6 = new Phrase("De plus, les congés annuels du salarié et les absences et/ou congés cumulés du bénéficiaire du service ne pourront excéder ", petit);
			Phrase conges7 = new Phrase("5 semaines annuellement ",
					petitgras);
			Phrase conges8 = new Phrase(" et non facturables. Une information devra impérativement être faite à l’association ", petit);
			...
			Paragraph facture3 = new Paragraph(new Phrase(
					" - des 5 semaines de congés et absences annuelles autorisées.",
					petit));
